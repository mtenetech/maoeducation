...
export default async function guardianRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (req, reply) => reply.send(await repo.list(req.params.id, req.user.institutionId)),
  )
  ...
  app.post<{ Params: { id: string }; Body: CreateGuardianDto }>(
    ...
    async (req, reply) =>
      reply.status(201).send(await repo.create(req.params.id, req.user.institutionId, req.body)),
  )
